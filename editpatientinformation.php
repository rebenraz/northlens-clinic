<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $stmtpatient->bind_param("i", $_GET['id']);
        $stmtpatient->execute();
        ...
        if ($patient) {
            ...
            $middleI = $patient['middle_initial'];
            ...
            if ($mh) {
                ...
                $others = $mh['others'];
                $last_visit = $mh['last_visit'];
                $eye_prescription = $mh['eye_prescription'];
                $existing_eye_glasses = $mh['existing_eye_glasses'];
                $url = $mh['file_url'];
            }
        }
    }
...
<body class="bg-white text-gray-800">
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelector('input[name="middleInitial"]').value = "<?php echo $middleI; ?>";
            ...
            const dob = new Date( "<?php echo $dob; ?>");
            ...
            document.querySelector('input[name="others"]').value = "<?php echo $others; ?>";
            document.querySelector('input[name="last_visit"]').value = "<?php echo $last_visit; ?>";
            document.querySelector('textarea[name="prev_perscription"]').value = "<?php echo $eye_prescription; ?>";
  ...
  <main class="max-w-5xl mx-auto p-6">
    ...
    <h2 class="text-lg font-semibold mb-2">Please complete the required information in the fields below.</h2>

    <!-- Patient Information -->
    <h3 class="text-xl font-bold mt-8 mb-4">Update Patient Information</h3>

    <form class="space-y-8" method="POST" action="crud/updatePatientInformation.php?id=<?php echo $_GET['id'] ?? 0 ?>" enctype="multipart/form-data">
      ...
      <!-- DOB and Phone -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex gap-2">
                <div style="width: 75%;">
                    <label class="block text-sm font-medium">Date Birth:</label>
                    <input type="date" name="dob" id="dob" required class="mt-1 border px-3 py-2 rounded w-full"/>
                </div>
                <div >
                    Age: 
    ...
    <!-- Left side -->
        <div class="space-y-4">
            ...
            <div>
            <label class="block text-sm font-medium">View Medical History:</label>
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <div class="flex gap-4 mt-1">
                    <label class="flex items-center gap-1">
                    <input type="radio" name="problems" required value="1" <?php echo ($problems == 1) ? 'checked' : ''; ?>  class="accent-blue-600" /> Yes
                    </label>
                    <label class="flex items-center gap-1">
                    <input type="radio" name="problems" required value="0" <?php echo ($problems == 0) ? 'checked' : ''; ?>  class="accent-blue-600" /> No
                    </label>
                </div>
            </div>
            <div>
                ...
                <div class="flex gap-4 mt-1">
                    <label class="flex items-center gap-1">
                        ...
                        required 
                        <?php echo ($existing_eye_glasses == 1) ? 'checked' : ''; ?> 
                        /> 
                        Yes
                    </label>
                    ...
                    <label class="flex items-center gap-1">
                        ...
                        class="accent-blue-600" 
                        required 
                        <?php echo ($existing_eye_glasses == 0) ? 'checked' : ''; ?> 
                        /> 
                        No
                    </label>
                    </div>
    ...
    <div class="flex items-center justify-between mt-8">
        <?php 
            if (isset($_GET['id'])) {
        ?>
            <button class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition">Update</button>
        <?php
            }
